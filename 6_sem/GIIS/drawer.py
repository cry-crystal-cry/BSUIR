...
class DrawerApp:
    ...
    def toggle_point_in_poly_mode(self):
        ...
        if self.point_in_poly_mode:
            self.selected_algorithm_label.config(text="Принадлежность точки полигону.")
    ...
    def on_left_click(self, event):
        if self.selected_fill_algorithm == "Flood":
            ...
            self.point_in_poly_mode = False
            if result:
                self.info_label.config(text="Точка принадлежит полигону.")
                return
            else:
                self.info_label.config(text="Точка не принадлежит полигону.")
                return
    ...
    def draw_parabola(self, a):
        x0, y0 = self.start_point
        points = bresenham_parabola(x0, y0, a, self.width, self.height)
